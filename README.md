...
Mini AutoSTAT 是一个基于 **LangGraph + langgraph-supervisor** 构建的问题驱动型多智能体统计分析系统（考核题目 B）。用户以自然语言提出分析假设，系统自动完成：数据体检 → 方法选择 → 统计分析（含受限代码生成与执行）→ 可视化 → 六要素结构化报告，全程支持用户中断、追加任务与断点续跑。
...
- **双层质量控制**：Leader 提示词内即时打回（最多 2 次）+ sync 节点确定性 JSON 校验，不合格自动回环
- **六要素报告**：数据说明、方法及选择原因、结果、不确定性、限制、不应得出的结论；LLM 成文 → 确定性校验 → 兜底降级三层保障，内容只引用真实运行结果
...
flowchart TD
    ...
    CP -- 用户追加任务 --> TEAM
    CP -- 确认出报告 --> REPORT[report 节点<br>六要素报告：LLM 成文→校验→兜底]
    GATE -- 预算/终止标记 --> REPORT
...
要点：**子图编译时显式 `checkpointer=False`**（嵌套子图继承父图 checkpointer 会引发 checkpoint 写死锁，见 Decision_Log D-020）；持久化只在外层图。
...
|--------|------|------|--------|-------------|
| **中断点** | team 每轮执行前 | 继续 | 终止并收敛到报告 | 注入新指令，Leader 重规划 |
...
```
mini_autostat/
├── app.py                        # 交互式 CLI 主入口（两类暂停点 + Ctrl+C 兜底）
├── requirements.txt              # 依赖清单
├── .env.example                  # 环境变量模板（复制为 .env）
├── agents/
│   ├── leader.py                 # Leader 提示词与 FINISH/打回上限常量
│   ├── reporter.py               # 六要素报告：素材收集→LLM 成文→校验→兜底
│   └── workers/
...
│   ├── report.md                 # 六要素报告
│   └── figures/*.png             # 图表
├── Decision_Log.md               # 全部架构/技术决策记录（D-001…D-025）
...
**输出**
- `outputs/report.md`：六要素结构化报告（LLM 成文；LLM 故障时自动降级为确定性兜底版并显式标注）
...
- **技术报告素材**：
  - 系统架构与设计取舍：本文档「系统架构」+ [Decision_Log.md](Decision_Log.md)（D-017~D-025 记录编排、checkpointer 死锁根因、中断机制、提供方切换、工具证据强制检查等真实决策链）
  - 失败案例：D-020 子图 checkpoint 死锁（五组对照实验定位）、D-016 IDE 缓冲区覆盖复发、M6 演示运行中的统计假设不满足→差分恢复链
  - 升级方向：[FUTURE_UPGRADES.md](FUTURE_UPGRADES.md)（六项，含 df 回写 Leader 审核）
