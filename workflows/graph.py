...
def build_app(config: AppConfig, tracer: RunTracer, checkpointer=None):
    ...
    def report(state: AnalysisState) -> dict:
        """出口节点：M4 最小确定性报告（M5 替换为 LLM 六要素报告）。"""
        lines = [
            "# Mini AutoSTAT 分析报告（M4 简版，M5 将升级为六要素报告）",
            f"\n## 假设\n{state.get('current_hypothesis', '(未记录)')}",
            f"\n## 终止原因\n{state.get('stop_reason')}",
        ]
        ...
        if state.get("statistical_results"):
            lines.append("\n## 统计结果")
            lines.append("```json\n" + json.dumps(
                state["statistical_results"], ensure_ascii=False, indent=1)[:3000]
                + "\n```")
        viz = state.get("visualizations") or []
        if viz:
            lines.append(f"\n## 图表（{len(viz)} 张）")
            lines += [f"- {v.get('title')}: {v.get('image_path')} ({v.get('status')})"
                      for v in viz]
        report_md = "\n".join(lines)
    ...
    g.add_conditional_edges(
        "gate",
        lambda s: "report" if s.get("stop_reason") else "team",
        {"team": "team", "report": "report"},
    )
    g.add_edge("report", END)

    # Checkpointer（D-020）：demo 用 SqliteSaver（MemorySaver+子图嵌套死锁，见 D-020）；
